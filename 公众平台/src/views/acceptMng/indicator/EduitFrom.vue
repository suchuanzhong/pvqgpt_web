...
export default {
  ...
  methods: {
    getInitData(){
      var _this = this;
      var url = "/BisIndexmonthreport/GetInfoById";
      _this.$fetchGet({ url, params: { id: _this.detailData.id } }).then(res => {
        _this.reportTime = res.data.yearNum + "-" +(res.data.monthNum > 10 ? res.data.monthNum : "0" + res.data.monthNum);
    ...
    submit() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid && _this.reportTime) {
          ...
          var url = "/BisIndexmonthreport/Update";
