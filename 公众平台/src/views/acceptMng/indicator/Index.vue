  ...
  methods: {
    ...
    deleForm(rowData) {
      this.$confirm("是否删除当前数据？", "提示", {
        ...
        var url = "/BisIndexmonthreport/DeleteData";
    ...
    queryData(page) {
      ...
      this.$fetch("/BisIndexmonthreport/QueryData",this.form).then(res => {
        this.tableData = res.data.data;
        this.tableData.forEach(item => {
          var qgl = 0;
          item.dataList.forEach(d => {
            if (d.itemType == 1 && d.dataValue != null) {
              qgl = qgl + parseFloat(d.dataValue);
            }
            if (d.itemKey == "tjzxss") {
              item.tjxss = d.dataValue;
            }
          });
          item.qgl = qgl;
        });
