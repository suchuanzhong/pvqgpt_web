...
<script>
    ...
    export default {
        ...
        created() {
            let barDatas=localStorage.getItem('dayEffic');

            this.dayEffic=JSON.parse(barDatas);
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            drawChart() {
                ...
                for (var i = 0; i < this.dayEffic.dataArr.length; i++) {
                    legend.push(this.dayEffic.dataArr[i].name);
                    if (this.dayEffic.dataArr[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.dayEffic.dataArr[i].type,
                        name: this.dayEffic.dataArr[i].name,
                        data: this.dayEffic.dataArr[i].data,
                        barMaxWidth: this.barWidth,
                        yAxisIndex: this.dayEffic.dataArr[i].yAxisIndex || 0,
                        tooltip: {
                            trigger: "axis",
                            axisPointer: {
                                type: "shadow"
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.dayEffic.dataArr.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.dayEffic.axisData.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.dayEffic.axisData.map(value => {
