...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('InverterAnalysis');
            this.InverterAnalysis=JSON.parse(barDatas);
            // this.is_table = this.InverterAnalysis.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            ...
            //     let _this = this
            //     _this.InverterAnalysis.yearsData.map((item,index)=>{
            //         tabledata.push({
            //             name:item,
            //             valueOne:_this.InverterAnalysis.datanalysis[0].data[index],
            //             valueTwo:_this.InverterAnalysis.datanalysis[1] == undefined?"": _this.InverterAnalysis.datanalysis[1].data[index],
            //             valueThree:_this.InverterAnalysis.datanalysis[2] == undefined?"":_this.InverterAnalysis.datanalysis[2].data[index],
            //             valueFour:_this.InverterAnalysis.datanalysis[3] == undefined?"":_this.InverterAnalysis.datanalysis[3].data[index],
            //         })
            //     })
            //     let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
            //     _this.InverterAnalysis.datanalysis.map((item,index)=>{
            //         let lableSTr = ""
            ...
            drawChart() {
                ...
                var yAxisLength = 0;
                var series = [], legend = [];
                for (var i = 0; i < this.InverterAnalysis.datanalysis.length; i++) {
                    legend.push(this.InverterAnalysis.datanalysis[i].name);
                    if (this.InverterAnalysis.datanalysis[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.InverterAnalysis.datanalysis[i].type,
                        name: this.InverterAnalysis.datanalysis[i].name,
                        data: this.InverterAnalysis.datanalysis[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data: this.InverterAnalysis.yearsData,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:16,
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.InverterAnalysis.datanalysis.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.InverterAnalysis.yearsData.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.InverterAnalysis.yearsData.map(value => {
