...
<script>
    export default {
        ...
        created() {
            let barDatas=localStorage.getItem('ConversionMore');
            this.ConversionMore=JSON.parse(barDatas);
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.ConversionMore.efficiency.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.ConversionMore.Conversion[0].data[index],
                        valueTwo:_this.ConversionMore.Conversion[1] == undefined?"": _this.ConversionMore.Conversion[1].data[index],
                        valueThree:_this.ConversionMore.Conversion[2] == undefined?"":_this.ConversionMore.Conversion[2].data[index],
                        valueFour:_this.ConversionMore.Conversion[3] == undefined?"":_this.ConversionMore.Conversion[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.ConversionMore.Conversion.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                for (var i = 0; i < this.ConversionMore.Conversion.length; i++) {
                    legend.push(this.ConversionMore.Conversion[i].name);
                    if (this.ConversionMore.Conversion[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.ConversionMore.Conversion[i].type,
                        name: this.ConversionMore.Conversion[i].name,
                        data: this.ConversionMore.Conversion[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data: this.ConversionMore.efficiency,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:14,
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.ConversionMore.Conversion.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.ConversionMore.efficiency.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.ConversionMore.efficiency.map(value => {
