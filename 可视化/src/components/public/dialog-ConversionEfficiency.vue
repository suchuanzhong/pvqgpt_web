...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('ConversionEfficiency');
            this.ConversionEfficiency=JSON.parse(barDatas);
            // this.is_table = this.ConversionEfficiency.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.ConversionEfficiency.efficiency.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.ConversionEfficiency.Conversion[0].data[index],
                        valueTwo:_this.ConversionEfficiency.Conversion[1] == undefined?"": _this.ConversionEfficiency.Conversion[1].data[index],
                        valueThree:_this.ConversionEfficiency.Conversion[2] == undefined?"":_this.ConversionEfficiency.Conversion[2].data[index],
                        valueFour:_this.ConversionEfficiency.Conversion[3] == undefined?"":_this.ConversionEfficiency.Conversion[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.ConversionEfficiency.Conversion.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                for (var i = 0; i < this.ConversionEfficiency.Conversion.length; i++) {
                    legend.push(this.ConversionEfficiency.Conversion[i].name);
                    if (this.ConversionEfficiency.Conversion[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.ConversionEfficiency.Conversion[i].type,
                        name: this.ConversionEfficiency.Conversion[i].name,
                        data: this.ConversionEfficiency.Conversion[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data: this.ConversionEfficiency.efficiency,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:14,
                                // fontSize: this.rotate && this.efficiency.length > 5 ? 12 : 14
                                // rotate:this.rotate&&this.efficiency.length>5?60:0
                                // interval:1
                            },
                            axisLine: {
                                show: true,
            ...
            getMaxSeriseData(data,index=1) {
                let newArr = data.map(item => {
                    if (item.type == "bar"&&item.yAxisIndex!=index) {
                        let Conversion = this.getMaxArr(item.data);
                        return Conversion.length > 0 ? Conversion[0] : 0;
                    } else {
                        return false;
                    }
                });
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.ConversionEfficiency.Conversion.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.ConversionEfficiency.efficiency.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.ConversionEfficiency.efficiency.map(value => {
