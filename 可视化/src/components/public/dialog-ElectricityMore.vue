...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('ElectricityMore');
            this.ElectricityMore=JSON.parse(barDatas);
            // this.is_table = this.ElectricityMore.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.ElectricityMore.Years.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.ElectricityMore.stents[0].data[index],
                        valueTwo:_this.ElectricityMore.stents[1] == undefined?"": _this.ElectricityMore.stents[1].data[index],
                        valueThree:_this.ElectricityMore.stents[2] == undefined?"":_this.ElectricityMore.stents[2].data[index],
                        valueFour:_this.ElectricityMore.stents[3] == undefined?"":_this.ElectricityMore.stents[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.ElectricityMore.stents.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                var yAxisLength = 0;
                var series = [], legend = [];
                for (var i = 0; i < this.ElectricityMore.stents.length; i++) {
                    legend.push(this.ElectricityMore.stents[i].name);
                    if (this.ElectricityMore.stents[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.ElectricityMore.stents[i].type,
                        name: this.ElectricityMore.stents[i].name,
                        data: this.ElectricityMore.stents[i].data,
                        barMaxWidth:this.barWidth,
                        stack: '总量',
                        yAxisIndex:0,// this.stents[i].yAxisIndex || 0,
                        tooltip: {
                            trigger: "axis",
                            axisPointer: {
                                type: "shadow"
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data: this.ElectricityMore.Years,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:16,
                                // rotate:this.rotate&&this.Years.length>5?60:0
                                // interval:1
                            },
                            axisLine: {
                                show: true,
            ...
            getMaxSeriseData(data,index=1) {
                let newArr = data.map(item => {
                    if (item.type == "bar"&&item.yAxisIndex!=index) {
                        let stents = this.getMaxArr(item.data);
                        return stents.length > 0 ? stents[0] : 0;
                    } else {
                        return false;
                    }
                });
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.ElectricityMore.stents.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.ElectricityMore.Years.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.ElectricityMore.Years.map(value => {
