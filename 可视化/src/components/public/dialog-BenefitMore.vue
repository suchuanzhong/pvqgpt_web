...
<script>
    export default {
        ...
        created() {
            ...
            // this.is_table=false;
            let barDatas=localStorage.getItem('BenefitMore');
           this.BenefitMore=JSON.parse(barDatas);
            // this.is_table =this.BenefitMore.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.BenefitMore.axisData.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.BenefitMore.dataArr[0].data[index],
                        valueTwo:_this.BenefitMore.dataArr[1] == undefined?"": _this.BenefitMore.dataArr[1].data[index],
                        valueThree:_this.BenefitMore.dataArr[2] == undefined?"":_this.BenefitMore.dataArr[2].data[index],
                        valueFour:_this.BenefitMore.dataArr[3] == undefined?"":_this.BenefitMore.dataArr[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.BenefitMore.dataArr.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                var yAxisLength = 0;
                var series = [], legend = [];
                for (var i = 0; i <this.BenefitMore.dataArr.length; i++) {
                    legend.push(this.BenefitMore.dataArr[i].name);
                    if (this.BenefitMore.dataArr[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type:this.BenefitMore.dataArr[i].type,
                        name:this.BenefitMore.dataArr[i].name,
                        data:this.BenefitMore.dataArr[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data:this.BenefitMore.axisData,
                            axisLabel: {
                                color: "#82c2eb",
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
               this.BenefitMore.dataArr.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas =this.BenefitMore.axisData.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex =this.BenefitMore.axisData.map(value => {
