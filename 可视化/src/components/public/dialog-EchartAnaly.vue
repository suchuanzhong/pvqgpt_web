...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('ComponentMore');
            this.ComponentMore=JSON.parse(barDatas);
            // this.ComponentMore=locData.barDatas;
            // this.is_table = this.ComponentMore.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.ComponentMore.priceData.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.ComponentMore.Componentprice[0].data[index],
                        valueTwo:_this.ComponentMore.Componentprice[1] == undefined?"": _this.ComponentMore.Componentprice[1].data[index],
                        valueThree:_this.ComponentMore.Componentprice[2] == undefined?"":_this.ComponentMore.Componentprice[2].data[index],
                        valueFour:_this.ComponentMore.Componentprice[3] == undefined?"":_this.ComponentMore.Componentprice[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.ComponentMore.Componentprice.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                for (var i = 0; i < this.ComponentMore.Componentprice.length; i++) {
                    legend.push( this.ComponentMore.Componentprice[i].name);
                    if ( this.ComponentMore.Componentprice[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type:  this.ComponentMore.Componentprice[i].type,
                        name:  this.ComponentMore.Componentprice[i].name,
                        data:  this.ComponentMore.Componentprice[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data:  this.ComponentMore.priceData,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:14,
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.ComponentMore.Componentprice.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas =  this.ComponentMore.priceData.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex =  this.ComponentMore.priceData.map(value => {
