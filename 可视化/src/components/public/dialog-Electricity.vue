...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('Electricity');
            this.Electricity=JSON.parse(barDatas);
            // this.is_table = this.Electricity.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.Electricity.Years.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.Electricity.stents[0].data[index],
                        valueTwo:_this.Electricity.stents[1] == undefined?"": _this.Electricity.stents[1].data[index],
                        valueThree:_this.Electricity.stents[2] == undefined?"":_this.Electricity.stents[2].data[index],
                        valueFour:_this.Electricity.stents[3] == undefined?"":_this.Electricity.stents[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.Electricity.stents.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                var yAxisLength = 0;
                var series = [], legend = [];
                for (var i = 0; i < this.Electricity.stents.length; i++) {
                    legend.push(this.Electricity.stents[i].name);
                    if (this.Electricity.stents[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.Electricity.stents[i].type,
                        name: this.Electricity.stents[i].name,
                        data: this.Electricity.stents[i].data,
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.Electricity.stents.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.Electricity.Years.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.Electricity.Years.map(value => {
