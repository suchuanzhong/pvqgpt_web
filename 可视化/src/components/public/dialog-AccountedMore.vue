...
<script>
    export default {
        ...
        created() {
            // this.is_table=false;
            let barDatas=localStorage.getItem('AccountedMore');
            this.AccountedMore=JSON.parse(barDatas);
            // this.is_table = this.AccountedMore.is_table
        },
        mounted() {
            this.$nextTick(()=>{
        ...
        methods: {
            tableInit(){
                ...
                let _this = this
                _this.AccountedMore.axisyearsData.map((item,index)=>{
                    tabledata.push({
                        name:item,
                        valueOne:_this.AccountedMore.dataAccounted[0].data[index],
                        valueTwo:_this.AccountedMore.dataAccounted[1] == undefined?"": _this.AccountedMore.dataAccounted[1].data[index],
                        valueThree:_this.AccountedMore.dataAccounted[2] == undefined?"":_this.AccountedMore.dataAccounted[2].data[index],
                        valueFour:_this.AccountedMore.dataAccounted[3] == undefined?"":_this.AccountedMore.dataAccounted[3].data[index],
                    })
                })
                let indexArr = ["valueOne","valueTwo","valueThree","valueFour"]
                _this.AccountedMore.dataAccounted.map((item,index)=>{
                    let lableSTr = ""
            ...
            drawChart() {
                ...
                var yAxisLength = 0;
                var series = [], legend = [];
                for (var i = 0; i < this.AccountedMore.dataAccounted.length; i++) {
                    legend.push(this.AccountedMore.dataAccounted[i].name);
                    if (this.AccountedMore.dataAccounted[i].yAxisIndex > 0 && yAxis.length == 1) {
                        yAxis.push({
                            name: this.unit[1],
                            type: "value",
                            splitNumber: 3,
                    ...
                    series.push({
                        type: this.AccountedMore.dataAccounted[i].type,
                        name: this.AccountedMore.dataAccounted[i].name,
                        data: this.AccountedMore.dataAccounted[i].data,
                ...
                let option = {
                    ...
                    xAxis: [
                        {
                            type: "category",
                            data: this.AccountedMore.axisyearsData,
                            axisLabel: {
                                color: "#82c2eb",
                                fontSize:14,
            ...
            initBg(option) {
                ...
                let yAxisIndex=0;
                this.AccountedMore.dataAccounted.forEach(item=>{
                    if(item.yAxisIndex==1){
                        yAxisIndex=1;
                    }
                })
                let maxData = this.getMaxSeriseData(option.series,1);
                let maxDatas = this.AccountedMore.axisyearsData.map(value => {
                    return maxData[0];
                });
                ...
                if(yAxisIndex==1){
                    ...
                    let maxDatasIndex = this.AccountedMore.axisyearsData.map(value => {
