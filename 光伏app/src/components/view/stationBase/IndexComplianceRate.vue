...
import EchartsLine from "@/components/public/EchartsLine.vue";
export default {
  components: {
    MtHeader,
    EchartsLine
  ...
  mounted() {
  this.getTimePowerGeneration("/AppApi/Base/GetTransformation");  
  },
  methods: {
     getTimePowerGeneration(url) {
        ...
        that.$fetch(url).then(res => {
            ...
            that.economicXAxis=batchdata.years
            // seriesData[0].name =batchdata.moreCrystalName
            seriesData[0].data =batchdata.moreCrystal
            // seriesData[1].name =batchdata.singleCrystalName
            seriesData[1].data =batchdata.singleCrystal
            // seriesData[2].name =batchdata.blackSiliconNamae
            seriesData[2].data =batchdata.blackSilicon
            // seriesData[3].name =batchdata.percName
            seriesData[3].data =batchdata.perc
            seriesData.forEach(item=>{
              
              item.data=item.data.map(citem=>{
               return citem == 0?citem='':citem
              })
            })
            console.log(11,seriesData)
